...
Generated by LevelFlow Pre-Auth Engine`,
  },
...
Generated by LevelFlow Pre-Auth Engine`,
  ...
  // ── p8: Medicaid PA (Texas TMHP format) ────────────────────────────────────
  p8: {
    patient_id: "p8",
    procedure_code: "D2750",
    clinical_summary: {
      patient_name: "Marvin Medicaid",
      date_of_birth: "1978-08-22",
      member_id: "TMHP-990-221-08",
      insurance_carrier: "Texas Medicaid (TMHP)",
      procedure_description: "Crown — Porcelain Fused to Metal (D2750), Tooth #19",
      tooth_numbers: ["#19"],
      treating_provider: "Dr. Sarah Patel, DDS",
    ...
    attached_files: [
      { filename: "pa_xray_tooth19.jpg",    file_type: "xray",  description: "Periapical radiograph showing extensive decay and pulpal involvement (#19)" },
      { filename: "perio_chart.pdf",         file_type: "chart", description: "Current periodontal charting — all sites stable" },
      { filename: "caries_risk_assessment.pdf", file_type: "report", description: "Caries risk assessment documenting high caries rate" },
    ],
    letter: `${new Date().toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}
...
RE: Prior Authorization Request — Crown, PFM (D2750)
    Patient: Marvin Medicaid  |  DOB: August 22, 1978  |  Medicaid ID: TMHP-990-221-08
    Tooth: #19  |  Proposed Date of Service: ${new Date(Date.now() + 7 * 86400000).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}

Dear TMHP Prior Authorization Review Committee:

I am requesting prior authorization for a porcelain-fused-to-metal crown (CDT Code D2750) on tooth #19 for my patient, Marvin Medicaid. This letter documents the medical necessity of this restoration per Texas Medicaid Dental Services policy.

CLINICAL FINDINGS

Tooth #19 (mandibular left first molar) presents with extensive mesio-occlusal-distal carious involvement that has compromised more than 60% of the clinical crown structure. The periapical radiograph confirms carious extension to within 1mm of the pulp chamber with no evidence of periapical pathology. Vitality testing confirms a vital pulp (positive to cold, negative to percussion). The existing amalgam restoration (placed approximately 8 years ago) has fractured along the lingual cusp, leaving insufficient tooth structure to support a direct restoration.

MEDICAL NECESSITY — TMHP CRITERIA

Per TMHP dental policy, crown placement is indicated when: (1) the remaining tooth structure cannot support a direct restoration, (2) radiographic evidence documents the extent of damage, and (3) the tooth is restorable. All three criteria are met. A direct composite or amalgam restoration would fail due to the extent of structural loss (>60% of crown compromised) and the fracture through the lingual cusp. The tooth is fully restorable with a full-coverage crown and does not require endodontic treatment.

SUPPORTING DOCUMENTATION ENCLOSED

  1. Periapical radiograph of tooth #19 — showing extent of caries and proximity to pulp
  2. Current periodontal charting — confirming periodontal health and no contraindication
  3. Caries risk assessment — documenting patient's elevated caries risk

Patient copay of $3.00 has been discussed per Texas Medicaid fee schedule. I respectfully request expedited review. Please contact our office at (555) 555-0100 with any questions.
...
Georgetown, TX 78626
NPI: 1234567890
Medicaid Provider #: TX-MCD-987654

Generated by LevelFlow Pre-Auth Engine — Texas Medicaid TMHP Format`,
  },
};
...
export async function POST(request) {
  ...
  const { patient_id, procedure_code, practice_name, practice_address, practice_phone, practice_npi, practice_tax_id } = body;
  ...
  // ── Determine account mode — only return mock data in sandbox ──────────────
  ...
  if (PYTHON_API_URL) {
    try {
      const upstream = await fetch(`${PYTHON_API_URL}/api/preauth/generate`, {
        ...
        body: JSON.stringify({ patient_id, procedure_code, practice_name, practice_address, practice_phone, practice_npi, practice_tax_id }),
  ...
  // ── Live mode without Python service → return error, not mock data ─────────
  if (accountMode !== "sandbox" && !PYTHON_API_URL) {
    return Response.json({
      error: "Pre-auth generation service not configured. Please contact your administrator.",
      error_type: "not_configured",
    }, { status: 503 });
  }

  // Mock fallback — sandbox mode only
  const mock = MOCK_RESPONSES[patient_id];

  // Slight delay to simulate real generation
  await new Promise(r => setTimeout(r, 1800));

  if (mock) {
    return Response.json({ ...mock, patient_id, procedure_code });
  }

  // No fixture match — generate a template letter using the procedure code
  const today = new Date().toLocaleDateString("en-US", { year:"numeric", month:"long", day:"numeric" });
  const CDT_NAMES = {
    D0120:"Periodic Oral Evaluation", D0150:"Comprehensive Oral Evaluation",
    D2750:"Crown, Porcelain Fused to Metal", D2751:"Crown, Porcelain",
    D3310:"Endodontic Therapy, Anterior", D3320:"Endodontic Therapy, Premolar", D3330:"Endodontic Therapy, Molar",
    D4341:"Periodontal Scaling and Root Planing, per quadrant",
    D5110:"Complete Denture, Maxillary", D5120:"Complete Denture, Mandibular",
    D6010:"Surgical Placement of Implant Body", D6065:"Implant-Supported Crown",
    D7140:"Extraction, Erupted Tooth", D7210:"Extraction, Surgical",
  };
  const procName = CDT_NAMES[procedure_code] || `Dental Procedure ${procedure_code}`;

  const pName = practice_name || "Practice Name";
  const pAddr = practice_address || "Address";
  const pPhone = practice_phone || "Phone";
  const pNpi = practice_npi || "NPI";
  const pTaxId = practice_tax_id || "Tax ID";

  const templateLetter = `PRIOR AUTHORIZATION REQUEST

Date: ${today}
To: Insurance Prior Authorization Department
From: ${pName}
Re: Prior Authorization for ${procName} (${procedure_code})

Dear Prior Authorization Review Committee,

I am writing to request prior authorization for ${procName} (CDT Code: ${procedure_code}) for our patient.

CLINICAL JUSTIFICATION:

The proposed treatment is medically necessary based on the following clinical findings:

1. Clinical examination reveals conditions warranting the proposed procedure
2. Conservative treatment alternatives have been considered and are insufficient
3. The proposed treatment represents the standard of care for the diagnosed condition

REQUESTED PROCEDURE:
- CDT Code: ${procedure_code}
- Description: ${procName}
- Estimated Fee: [See attached treatment plan]

Supporting documentation including radiographs, clinical photographs, and periodontal charting are attached for your review.

I respectfully request expedited review of this authorization. Please contact our office at ${pPhone} if additional information is needed.
...
Generated by LevelFlow Pre-Auth Engine`;
  ...
  return Response.json({
    ...
    attached_files: [],
    clinical_summary: { procedure_description: procName, procedure_code },
  });
}
