...
function odAuth() {
  const dev  = process.env.OD_DEV_KEY      || OD_DEMO_DEV_KEY;
  const cust = process.env.OD_CUSTOMER_KEY || OD_DEMO_CUST_KEY;
...
function isDemoMode() {
  const dev  = process.env.OD_DEV_KEY      || OD_DEMO_DEV_KEY;
  const cust = process.env.OD_CUSTOMER_KEY || OD_DEMO_CUST_KEY;
  return dev === OD_DEMO_DEV_KEY && cust === OD_DEMO_CUST_KEY;
}
